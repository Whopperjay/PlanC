...
def sanitize_drivers():
    """
    Ensures all required non-optional fields are present for every driver in drivers.json.
    Swift's APIDriver declares dateOfBirth and nationality as non-optional String:
    any missing field causes JSONDecoder to throw, which makes drivers=[] and breaks predictions.

    Also assigns a placeholder permanentNumber (99, 98, ...) to drivers without one,
    so they can be matched in OpenF1 sprint-qualifying lookups.
    """
    ...
    # Collect already-used permanent numbers to avoid collisions
    used_numbers = {str(d['permanentNumber']) for d in drivers if d.get('permanentNumber')}
    ...
    for d in drivers:
        if not d.get('dateOfBirth'):
            ...
            print(f"  sanitize_drivers: filled dateOfBirth for {d['driverId']}")
            changed = True
        if not d.get('nationality'):
            d['nationality'] = 'Unknown'
            print(f"  sanitize_drivers: filled nationality for {d['driverId']}")
            changed = True
        if not d.get('permanentNumber'):
            while str(placeholder) in used_numbers:
                placeholder -= 1
            d['permanentNumber'] = str(placeholder)
            used_numbers.add(str(placeholder))
            placeholder -= 1
            print(f"  sanitize_drivers: assigned placeholder number {d['permanentNumber']} to {d['driverId']}")
            changed = True

    if changed:
        data['MRData']['DriverTable']['Drivers'] = drivers
        with open(path, 'w') as f:
            json.dump(data, f, indent=2)
        print(f"  sanitize_drivers: saved {len(drivers)} drivers")
    else:
...
def generate_session_overrides():
    ...
    Keys match F1Event.id in the bundled f1_2026_calendar.json.
    Times are UTC ISO8601 strings (e.g. '2026-03-28T06:00:00Z').
    Sprint Qualifying (shootout) is NOT in Ergast -> keeps bundled time.
    """
    path = os.path.join(DATA_DIR, 'current_schedule.json')
    if not os.path.exists(path):
        return
    ...
    for race in data['MRData']['RaceTable']['Races']:
        ...
        for ergast_key, suffix in ERGAST_SESSION_SUFFIX.items():
            session = race.get(ergast_key)
            if session and session.get('date') and session.get('time'):
                sessions[prefix + '-' + suffix] = session['date'] + 'T' + session['time']

        if race.get('date') and race.get('time'):
            sessions[prefix + '-gp'] = race['date'] + 'T' + race['time']

    out_path = os.path.join(DATA_DIR, 'calendar_sessions.json')
    with open(out_path, 'w') as f:
        json.dump({'sessions': sessions}, f, indent=2)
    print('  generate_session_overrides: ' + str(len(sessions)) + ' session times saved')


def generate_calendar_status():
    """
    Generates data/calendar_status.json listing confirmed and cancelled races.
    A race in ERGAST_TO_EVENT_PREFIX not in current Ergast schedule = cancelled.
    The app uses this to update F1Race.cancelled at runtime without a rebuild.
    """
    path = os.path.join(DATA_DIR, 'current_schedule.json')
    if not os.path.exists(path):
        return
    ...
    with open(out_path, 'w') as f:
        json.dump({'confirmed': confirmed, 'cancelled': cancelled}, f, indent=2)
    print('  generate_calendar_status: ' + str(len(confirmed)) + ' confirmed, ' + str(len(cancelled)) + ' cancelled')
...
def job():
    ...
    if success_count > 0:
        ...
        # Sanitize driver fields (fill missing non-optional fields like dateOfBirth, nationality)
        sanitize_drivers()
    ...
    try:
        ...
        for session in sessions:
            ...
            for drv_num, pos_val in sorted_drivers:
                ...
                constructor = driver_to_constructor.get(driver_id, {"constructorId": "", "url": None, "name": "", "nationality": ""})
...
def fetch_driver_teams():
    ...
    try:
        ...
        for constructor in constructors:
            ...
            d_url = f"https://api.jolpi.ca/ergast/f1/current/constructors/{c_id}/drivers.json"
